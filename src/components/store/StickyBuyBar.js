...
// the price is never more than a tap from wherever the reader is. It steps
// aside while the price card or the final buttons are on screen, and it links
// to the price card rather than starting a checkout of its own.
    ...
    <div className={`sticky-buy${show ? ' sticky-buy-on' : ''}`} inert={!show}>
      <div className="sticky-buy-inner">
        ...
        <span className="sticky-buy-name">
          {product.name}
          <span className="sticky-buy-price"><Price product={product} />, paid once</span>
        </span>
        <a href="#buy" className="store-btn store-btn-primary">Buy</a>
      </div>
    </div>
  );
}
